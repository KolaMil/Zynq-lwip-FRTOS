#pragma once

#include "FreeRTOS.h"
#include "queue.h"
#include "task.h"
#include "message_buffer.h"

#include <stdlib.h>
#include <stdbool.h>
#include <arm_neon.h>

#include "lwip/tcp.h"
#include "lwip/inet.h"
#include "lwip/udp.h"
#include "lwip/init.h"
#include "xparameters.h"
#include "xil_types.h"
#include "xil_printf.h"
#include "xil_io.h"
#include "lwip/pbuf.h"
#include "tcp_stack.h"
#include "udp_stack.h"
#include "cmd.h"
//#include "asterix.h"
//#include "auto_gain_control.h"

/*-----------------------------------------------------------*/
#define LOCAL_IP_ADDRESS    "192.168.1.10"
#define REMOTE_IP_ADDRESS   "192.168.1.100"
#define LOCAL_NET_MASK      "255.255.255.0"
#define LOCAL_GATEWAY       "192.168.1.1"
#define TCP_REMOTE_PORT     ((uint16_t)7)
#define UDP_REMOTE_PORT     ((uint16_t)5005)
#define TCP_PORT 7

/*-----------------------------------------------------------*/
struct udp_pcb *udp_pcb_conn;
struct udp_pcb *udp_pcb_answer;
struct tcp_pcb *tcp_pcb;
struct netif server_netif;
struct pbuf *cat240_pbuf;
struct pbuf *fast_for_main_pbuf;
udp_sender_t *sender;
tcp_monitor_t *monitor;
//cat240_message_t *metainfo_parser;
//cat240_storage_t *storage;
//cat240_storage_t *storage_massive[10];
extern volatile QueueHandle_t xTcpMsgQueue;
extern volatile TaskHandle_t xIrqTaskHandle;
extern volatile TaskHandle_t xIrqUDPTaskHandle;
extern volatile QueueHandle_t xPbufQueue;
extern volatile MessageBufferHandle_t xMsgBuffer;
extern volatile MessageBufferHandle_t xPacketBuffer;
extern uint64_t debug_counter;
extern uint8_t default_state[8192];
extern uint8_t try_reconnect;
extern uint8_t work_size;
extern uint8_t old_work_size;
//extern bool extend_pack;
//extern bool starting;
//extern bool update_mode;

uint8_t local_buf[8192];
uint8_t mode;
/*-----------------------------------------------------------*/
void lwip_network_setup(void);
err_t udp_connection(uint8_t *data, size_t size);
//void compose(uint8_t* src, cat240_storage_t* store);
//void create_massive(cat240_storage_t **ptr, size_t size, size_t data_size);
//void storages_initializ(uint16_t size_);
//void free_storage_massive(cat240_storage_t **ptr, size_t size);
//void clean_all(void);
//void refactor(cat240_storage_t** dir, size_t size);
void udp_receive_callback(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port);
err_t tcp_connection_cl(void);
err_t udp_package_send(void);
err_t tcp_client_close(struct tcp_pcb *pcb);
err_t client_connected(void *arg, struct tcp_pcb *tpcb, err_t err);
int parse_msg(void* p, size_t size);
err_t need_reconnect(struct tcp_pcb *pcb);
err_t reconnection_tcp(struct tcp_pcb *pcb);
err_t recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err);
